...
function changeLanguage(lang: string, link?: string) {
  if (lang !== locale.value) {
    const path = page.value.relativePath === 'index.md' ? '/' : page.value.relativePath.substring(locale.value === 'root' ? 0 : (locale.value as string).length)
...
<template>
  <div class="header-suffix">
    ...
    <!-- <Dropdown class="language" :transfer="false">
      ...
      <template #drop>
        <DropdownList>
          <DropdownItem v-for="option in langOptions" :key="option.lang" :name="option.name" :selected="option.lang === locale" @select="changeLanguage(option.lang, option.link)">
            {{ option.name }}
          </DropdownItem>
        </DropdownList>
      </template>
    ...
    <Linker class="github-link" to="//github.com/appnify/arconify" :title="t('common.viewInGithub')">
