...
export default defineConfig(({ command }: ConfigEnv): any => {
  ...
  return <UserConfigExport>{
    ...
    optimizeDeps: {
      include: ['@vexip-ui/icons', ...Object.keys(pkg.dependencies ?? {}).filter((dep: string) => !dep.includes('vexip-ui'))],
    },
    esbuild: {
    ...
    plugins: [vueJsx(), i18n({ include: resolve(__dirname, './vitepress/i18n') }), inspect(), demoImports()],
  }
})
